      ...
      <div class="center bg">
          ...
          <div class="center formarea">
              ...
              <div class="codearea">
                   ...
                   <van-button size="small" class="codeBtn" @click="sendCode">重获验证码{{timeout}}</van-button>
              </div>
              <div class="button" @click="getCoupon2"><img src="./btn.png"></div>
  ...
  methods: {
   ...
   getCoupon2() {
     ...
     //this.popupShow=true; //显示输入验证码框
      ...
      axios.post(this.URL, qs.stringify(request)).then(response => {
          toast.clear();
           ...
           axios.post(this.URL2,qs.stringify(request)).then(response => {
               ...
               if (response.data.success) {
                   ...
                   //this.popupShow=false; //关闭输入验证码框
                       ...
                       // alert(action);
                        location.href=' https://8.m.tuniu.com/xdm/m/index/nsf/agreement/0/2';
                        //alert("好");                    
                        })
...
.texttitle a{text-decoration:underline; color: #fff}
.textcontent {
  padding: 6px 10px;
  font-size: 12px;
  color: #fff;
